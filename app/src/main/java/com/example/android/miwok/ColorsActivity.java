...
public class ColorsActivity extends AppCompatActivity {
        ...
        listView.setOnItemClickListener (new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                if (res = AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
                    releaseMediaPlayer();
                    mediaPlayer = MediaPlayer.create(ColorsActivity.this, currentWord.getSound());
                    mediaPlayer.start();
                    ...
                    AudioManager.OnAudioFocusChangeListener afChangeListener = new AudioManager.OnAudioFocusChangeListener() {
                                ...
                                public final void onAudioFocusChange(int focusChange) {
                                    switch (focusChange) {
                                        case focusChange == AudioManager.AUDIOFOCUS_LOSS:
                                            // Permanent loss of audio focus
                                            mediaPlayer.stop();

                                        case focusChange == AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                                            mediaPlayer.pause();

                                        case focusChange == AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
                                            mediaPlayer.pause();

                                        case focusChange == AudioManager.AUDIOFOCUS_GAIN:
                                            mediaPlayer.start();
                                    }
                                }
                            };
    ...
    private void requestAudioFocus(AudioManager.OnAudioFocusChangeListener l, AudioManager.STREAM_MUSIC,
                                   AudioManager.AUDIOFOCUS_GAIN_TRANSIENT){
        final int res = audioManager.requestAudioFocus(afChangeListener, AudioManager.STREAM_MUSIC, // Music streaming
                AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
